...
const messagesElQuery =
  '[class^=styles_scrollable-div] > .MuiGrid-item:not([data-uuid])';

let observeTimer: ReturnType<typeof setInterval> | null = null;

export const observeMessages = () => {
  observeTimer = setInterval(() => {
    const allMessagesElements: NodeListOf<HTMLDivElement> =
      document.querySelectorAll(messagesElQuery);
